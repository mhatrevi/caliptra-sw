...
fn real_main() -> Result<(), Box<dyn Error>> {
    ...
    if args.len() != 4 {
        Err("Usage: codegen [--check] <caliptra_ss_dir> <extra_rdl_dir> <dest_dir>")?;
    }

    let caliptra_ss_dir = Path::new(&args[1]);
    let rtl_dir = caliptra_ss_dir.join("third_party").join("caliptra-rtl");
    let i3c_core_rdl_dir = caliptra_ss_dir.join("third_party").join("i3c-core");
...
`include "mcu_mbox_csr.rdl""#,
    ...
    write!(
        &mut header,
        "\n generated by caliptra_registers_generator from:"
    )?;
    for (name, path) in [
        ("caliptra-ss", caliptra_ss_dir),
        ("caliptra-rtl", rtl_dir.as_path()),
        ("adams-bridge", adamsbridge_rdl_dir.as_path()),
        ("i3c-core", i3c_core_rdl_dir.as_path()),
    ] {
        let commit_id = run_cmd_stdout(
            Command::new("git")
                .current_dir(path)
                .arg("rev-parse")
                .arg("HEAD"),
        ...
        if !git_status.is_empty() {
            write!(&mut header, "\n Warning: {name} was dirty:{git_status}")?;
        }
    }
